...
[Starlight Markdown](https://starlight.astro.build/guides/authoring-content/)
(asides, plain `<details>` blocks, etc.), plus a few small local extensions
(see `docs/plugins/`): `:::tab[Label]` directives for content tabs,
`{% include "..." %}` inside code fences to pull in runnable snippets from
`docs/code_snippets/`, and a `` `#!python ...` `` prefix for
syntax-highlighted inline code. The landing page, changelog and
