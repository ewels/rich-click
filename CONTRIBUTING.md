...
(asides, plain `<details>` blocks, etc.), plus a couple of small local
extensions: `:::tab[Label]` directives for content tabs and
`{% include "..." %}` inside code fences to pull in runnable snippets from
`docs/code_snippets/` (see `docs/plugins/`). The landing page, changelog and
