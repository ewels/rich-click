...
- Added **machine-readable help formats** on the existing `--help` flag, so tooling and LLM agents can discover a CLI's structure as data. The capability is always available — no config needed — and a bare `--help` is unchanged:
  ...
  - Added an **AI hint** — a dimmed tip under regular `--help` pointing coding agents at `--help markdown`. `show_ai_markdown_hint` defaults to auto mode using agent-environment detection; set it to `True` to always show or `False` to always hide. Customize it via `ai_markdown_hint_text` and `style_ai_markdown_hint`.
