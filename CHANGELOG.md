...
- Added `align_columns_across_panels` config option, on by default, which lines up the columns of every panel with each other instead of sizing each panel's table on its own. A column is sized by the entries that reach past it, so an option with no short form or metavar runs on under those columns rather than widening the one it is in. Set it to `False` for the previous layout. [[#34](https://github.com/ewels/rich-click/issues/34)]
- Deprecated `style_commands_table_column_width_ratio`, which truncates a long command name to hold a fixed proportion; `wrap_long_options` and `align_columns_across_panels` now cover what it was for. Setting it raises a `DeprecationWarning` and keeps command panels out of the alignment pass, so that an explicit ratio still decides their column widths. [[#111](https://github.com/ewels/rich-click/issues/111)]
- Added `wrap_long_options` config option, defaulting to `40` characters, which keeps an entry whose columns before the help are wider than that from setting the column width for a whole panel, and caps how wide those columns grow to hold an entry spilling across them. Such an entry first spills into the columns it leaves empty to its right, and moves its help text to the line below when that is not room enough. Pass a `float` instead for a fraction of the panel's width, or a callable taking the panel's width and returning either. Set it to `0`, a negative number, `False` or `None` to turn it off.
- `align_columns_across_panels` and `wrap_long_options` both change the default help output of every CLI, so snapshot tests of help screens will need updating.
