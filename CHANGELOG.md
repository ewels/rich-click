...
- Added **machine-readable help formats** on the existing `--help` flag, so tooling and LLM agents can discover a CLI's structure as data. The capability is always available — no config needed — and a bare `--help` is unchanged for human users:
  - `--help markdown` (alias `--help md`) and `--help markdown-full` (`--help md-full`) — LLM-friendly Markdown: headings for hierarchy, each command titled by its full invocation path, and parameters as compact tables. `markdown-full` documents the whole tree; `markdown` uses **adaptive disclosure** (below).
  - **Adaptive disclosure**, shared by `--help markdown` and the agent-facing bare `--help`: the invoked command is documented in full, and the rest of the tree is disclosed as far as a **character** ceiling allows — promoting commands breadth-first (nearest hop first) from a name-only pointer, through a bare option-signature list, to full detail. The default ceiling is high enough that small and mid-size CLIs return their **entire** tree in full from a single `--help`, so an agent never has to guess which subcommand holds the flag it needs; only very large CLIs degrade, and they degrade gracefully rather than falling back to bare names. The ceiling is the `agent_help_max_chars` config option / `AGENT_HELP_MAX_CHARS` global, default 25,000 (`None` disables adaptation). Characters, not tokens, because the failure the ceiling prevents is an agent harness truncating the output, and harness caps are measured in characters (Claude Code cuts a tool result at ~30,000) — `len()` also makes the ceiling exact, where a token estimate needed a per-format characters-per-token divisor. A whole-tree format (`--help compact`, `markdown-full`, `json`, `json-full`, `carapace`) is unaffected.
  - `help_to_stderr` now applies to the machine-readable formats too, not just the human help — otherwise a `--help json` (or an agent-redirected bare `--help`) leaked into the very stdout the setting exists to keep clean.
  - `--help json` no longer drops the default of a flag that defaults to **True**: `--debug/--no-debug` with `default=True` used to serialize identically to one defaulting to `False`. A flag defaulting to `False` is still omitted, since that is the implied case.
  - `--help carapace` no longer marks value-less options as taking a value: a counter (`-v/-vv`) is emitted as a bare key rather than `-v, --verbose=`, and `--help` — whose value is optional — uses carapace's `?` suffix instead of `=`.
  - A usage-error diagnosis that raises is now swallowed rather than propagating, so an enhancement can never replace the error message the user actually needs to read.
  - Commands marked `hidden=True` are omitted from the Markdown and compact renderings, matching the rendered help screen — previously they were listed, and under adaptive disclosure a hidden command's full option table could be documented. The JSON formats still report them, as `to_info_dict()` and hidden *parameters* do.
  - Markdown parameter tables now **drop any column that is empty for every row** (typically Required and Default) instead of rendering it as a run of empty cells. This applies to `--help markdown` and `--help markdown-full` alike, so their tables are narrower than in previous builds of this feature; no information is lost.
  ...
  - `--help carapace` — output conforming to the [carapace](https://carapace.sh) completion spec, making rich-click a producer for that ecosystem. Emitted as YAML (install the `rich-click[carapace]` extra for `pyyaml`); without it, falls back to JSON, which carapace also accepts.
  - `--help compact` — the whole command tree in the leanest complete form, and **what a bare `--help` now renders for a detected agent**. One line per record with the signature and description separated by exactly two spaces; a `#` anchor line opening each command's block; `*` for a required option, `a|b|c` inline for a choice, `...` for a repeatable one, and Click's own `[default: x]` / `[env: NAME]` tags; a `usage:` line only where positional arguments make order meaningful; and no `--help` row. It uses only conventions a model already reads in rendered help — no invented sigils — and measures ~4x smaller than the equivalent Markdown, which is what keeps a mid-size CLI inside a single agent tool response. **The output shape changed from the earlier experimental build of this feature** (which was `--name METAVAR (notes) — description` lines under `Options:` headings); anything parsing it should be updated.
    - Asked for by name it renders every command in the tree with no ceiling; reached as the agent default it runs under adaptive disclosure instead, with compact-native tiers.
  ...
  - The machine-readable output reports the `--help` option itself (like the rendered help screen does), with the formats it accepts surfaced as its `choices`, so an agent can discover them from the data. The one exception is `--help compact`, which omits the row as boilerplate repeated on every command.
  - The schema is built from each command's `to_info_dict()`, so anything a developer adds there flows through automatically. Output can be post-processed with the `help_json_transform` hook, or reshaped by overriding the `format_help_*` methods. New formats can be registered either via the `help_formats` class attribute (subclass) or the `help_formats` config option / `HELP_FORMATS` global (a `{name: (command, ctx) -> str}` map, no subclass needed).
  - **Automatic agent detection**: inside an AI coding agent (detected from the environment variables agents set, including the emerging `AI_AGENT` and `AGENT` conventions), a bare `--help` renders the machine-readable format directly instead of the help screen — no hint for the agent to notice, and no second call. The format is the `agent_help_format` config option / `AGENT_HELP_FORMAT` global (default `compact`; set it to `markdown` for the Markdown rendering, or `None` to opt out), and an explicit `--help <format>` is always honoured as given.
    - Detection can be forced either way with `RICH_CLICK_AGENT_MODE=true|false`, and is automatically suppressed when `PYTEST_CURRENT_TEST`, `PYTEST_VERSION` or `RICH_CODEX` is present, so downstream `--help` snapshot tests and screenshots generated by [rich-codex](https://ewels.github.io/rich-codex/) >= 1.3.1 stay human-readable even when run from inside an agent.
- Added **error diagnosis** for usage errors: instead of only reporting the symptom (`No such option: --repo`), rich-click works out the rule that was broken and says so. It derives the violated rule — including the parent-group case (`'--repo' is an option of the parent group 'tool', not of 'tool build'`) — near matches by edit distance over the command's real option and subcommand names, the valid values of a `Choice`, and a copyable corrected invocation where one can be built confidently.
  - Humans get a terse addition inside the existing error panel; in a detected AI agent environment the error is rendered as a plain-text block instead — no ANSI, one fact per line, with the attempted invocation restated. Same detection machinery as agent help.
...
- Added an `examples=` argument to commands and groups: a list of `(description, command)` tuples (description first, and required). Examples render in every output: an **Examples** panel in the human `--help`, an `## Examples` section in `--help markdown`, an `examples` array in `--help json`, and the `examples` map in `--help carapace`. The panel title is configurable via `examples_panel_title`.
  - In the agent-facing Markdown formats the `## Examples` section sits immediately after the usage line, **before** the parameter tables — an example is a complete, copyable invocation, so it goes ahead of the reference material. Commands abbreviated by adaptive disclosure keep their examples in full. The rendered human `--help` is unchanged, with its Examples panel after the options.
