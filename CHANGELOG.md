...
- Added **machine-readable help formats** on the existing `--help` flag, so tooling and LLM agents can discover a CLI's structure as data. The capability is always available — no config needed — and a bare `--help` is unchanged for human users:
  ...
  - **Adaptive disclosure** in `--help markdown`: the invoked command is documented in full, and the rest of the tree is disclosed as far as an approximate token ceiling allows — promoting commands breadth-first (nearest hop first) from a name-only pointer, through a compact option-signature list, to full detail. The default ceiling is high enough that small and mid-size CLIs return their **entire** tree in full from a single `--help`, so an agent never has to guess which subcommand holds the flag it needs; only very large CLIs degrade, and they degrade gracefully rather than falling back to bare names. The ceiling is the `agent_help_max_tokens` config option / `AGENT_HELP_MAX_TOKENS` global (`None` disables adaptation); size is estimated from a characters-per-token ratio rather than a tokenizer, so `--help` stays fast and dependency-free. `--help markdown-full`, `json`, `json-full` and `carapace` are unaffected.
  ...
  - `--help compact` — **experimental**: the leanest rendering of the same data, one line per option (`--name METAVAR (notes) — description`) and one line per subcommand, with no tables. Explicitly requested only; never a default in any environment. It exists so a token-lean and a familiar rendering can be compared over identical content.
  ...
  - The machine-readable output reports the `--help` option itself (like the rendered help screen does), with the formats it accepts surfaced as its `choices`, so an agent can discover them from the data.
